...
Answer: There are 240 observations. 
...
This means that there was no data found for that variable.
...
```{r plastic_waste_per_cap-dens-color-fill-alpha2, eval = TRUE}
...
Answer: Color and fill are in the mapping aesthetic because it is dependent on the x and y coordinates. It is a different color per continent. The alpha level is standard, though, as the alpha level applies to the whole graph, regardless of the continent. 
...
ggplot(data = plastic_waste, 
  ...
  geom_boxplot()+
  labs(
    title = "Boxplot of Plastic Waste per Capita",
    x = "Continent",
...
ggplot(data = plastic_waste, 
  ...
  geom_violin()+
  labs(
    title = "Violin Plot of Plastic Waste per Capita",
    x = "Continent",
    y = "Plastic Waste per Capita"
  )
```

Answer: The violin plot reveals the distribution within the box. It shows if they are closer to 0 (Africa and South America) or closer to the top (Europe and North America). 
...
```{r plastic_waste_per_cap-scatter, eval = TRUE}
ggplot(data = plastic_waste, aes(x = plastic_waste_per_cap, y=mismanaged_plastic_waste_per_cap)) +
  geom_point()+
  labs(
    title = "Plastic Waste",
    subtitle = "Plastic Waste vs. Mismanaged Plastic Waste",
    x = "Plastic Waste Per Capita",
    y = "Mismanaged Plastic Waste Per Capita"
  )
```

The relationship shows a positive trend. The more plastic waste per capita aligns with more mismanaged plastic waste per capita. 
...
```{r plastic_waste_per_cap-scatter-color, eval = TRUE}
ggplot(data = plastic_waste, aes(x = plastic_waste_per_cap, y=mismanaged_plastic_waste_per_cap, color = continent, fill = continent)) +
  geom_point()+
  labs(
    title = "Plastic Waste",
    subtitle = "Plastic Waste vs. Mismanaged Plastic Waste",
...
North America tends to have a higher waste per capita and mismanaged waste. Europe has a low mismanaged waste but a higher plastic waste per capita. Africa has a low plastic waste per capita but a higher mismanaged waste. 
...
```{r plastic_waste_per_cap-scatter-population, eval = TRUE}
ggplot(data = plastic_waste, aes(x = plastic_waste_per_cap, y= total_pop, color = continent, fill = continent)) +
  geom_point()+
  labs(
    title = "Plastic Waste",
    subtitle = "Plastic Waste vs. Population",
    x = "Plastic Waste Per Capita",
    y = "Total Population"
  )
```

```{r plastic_waste_per_cap-scatter-coast, eval = TRUE}
ggplot(data = plastic_waste, aes(x = plastic_waste_per_cap, y=coastal_pop, color = continent, fill = continent)) +
  geom_point()+
  labs(
    title = "Plastic Waste",
    subtitle = "Plastic Waste vs. Coastal Population",
    x = "Plastic Waste Per Capita",
    y = "Costal Population"
  )
```
...
```{r plastic_waste_copy, eval = TRUE}
plastic_waste %>%
  mutate(coastal_pop_prop = coastal_pop / total_pop) %>%
  filter(plastic_waste_per_cap < 3) %>%
  ggplot(aes(x = coastal_pop_prop, y = plastic_waste_per_cap, color = continent)) +
  ...
  theme_minimal()
```
Interpretation: This graph shows the the percent of the population that lives on the coast and how much plastic waste in generated per capita. This can show the relationship between coastal living continents and plastic waste. Continents like Oceania have a higher coastal population and a wide range of plastic waste per capita. Africa has a lower coastal living and lower plastic waste per capita. The black line shows the average line of the points, so you can tell if points are above or below the average. 
